...
from app.GHS_pic.ghs_pic import hso_pic,add_setu
...
def if_send_like(msg, Qnum):
    if msg in ["给我点赞", "点赞", "赞我", "点名片赞", "自动点赞"]:
        return [True, "很抱歉，换了新的框架，hex酱不再会点赞了呢~"]
    return [False]
...
def talk_to_user(rev, talk_data):
    ...
    if msg_img[0] == False:  # 出错就返回报错内容
        return msg_img[1]
    if msg_img[0] == True:
        if_add_setu = add_setu(msg_img[1], msg_img[2])
        if if_add_setu[0] == True:
            return if_add_setu[1]
